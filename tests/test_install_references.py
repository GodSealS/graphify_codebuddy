...
def test_built_wheel_ships_the_full_skill_payload():
    ...
    # CodeBuddy deliberately reuses skill.md + the claude bundle. A second body
    # and references tree would be dead package payload that can drift.
    assert "graphify/skill-codebuddy.md" not in names
    assert not any(name.startswith("graphify/skills/codebuddy/") for name in names)
