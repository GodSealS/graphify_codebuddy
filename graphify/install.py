...
def codesquad_install(project_dir: Path | None = None) -> None:
    ...
    _copy_skill_file("codesquad", project=bool(project_dir), project_dir=project_dir)
    # Write to .codesquad/AGENTS.md (CodeSquad-style agents routing file)
    target = (project_dir or Path(".")) / ".codesquad" / "AGENTS.md"
    ...
    print()
    print("Project-scoped install. Add to version control:")
    print(f"  git add .codesquad/skills/graphify/")
    print(f"  git add .codesquad/AGENTS.md")


def codesquad_uninstall(project_dir: Path | None = None, *, project: bool = False) -> None:
    """Remove the graphify skill tree and the .codesquad/AGENTS.md section."""
    project_dir = project_dir or Path(".")
    _remove_skill_file("codesquad", project=project, project_dir=project_dir)
...
def dispatch_install_cli(cmd: str) -> bool:
    ...
    if cmd == "install":
        ...
        if subcmd == "install":
            if "--project" in sys.argv[3:]:
                _project_install("codesquad", Path("."))
            else:
                codesquad_install()
        elif subcmd == "uninstall":
            if "--project" in sys.argv[3:]:
                _project_uninstall("codesquad", Path("."))
            else:
                codesquad_uninstall()
        else:
